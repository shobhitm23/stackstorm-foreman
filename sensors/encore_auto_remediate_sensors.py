...
class EncoreAutoRemediateSensor(PollingSensor):
    def __init__(self, sensor_service, config=None, poll_interval=None):
        super(EncoreAutoRemediateSensor, self).__init__(
    ...
    def poll(self):
        ...
        for query in queries:
            ...
            timeline = query_date
            query_date = int(timeline.split()[0])

            for obj in result:
                if "subscription_facet_attributes" in obj:
                    server_checkin_date = int(
                        obj["subscription_facet_attributes"]["last_checkin"].split()[0]
                    )
